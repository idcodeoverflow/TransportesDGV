 ...
 * @author David
 */
public class CargoEspecialReport extends ExcelReport{
    
    private Timestamp fechaInicio;
    private Timestamp fechaFin;

    public CargoEspecialReport(javax.swing.JFrame form){
        super(form);
    }
    
    public CargoEspecialReport(){
        super(null);
    }
    ...
    public void generarReporte() {
        ...
        try {
            cargos = accesoCargos.obtenerCargosEspeciales(true);
            lazyQ.startLazyQuery();
            for (CargoEspecialDTO cargo : cargos) {
                fila = sheet.createRow(nFila++);

                mensajeError = cargo.toString();

                Cell celdaT = fila.createCell(0);
                celdaT.setCellValue(cargo.getIdCargoEspecial());
                ...
                celdaT.setCellValue(cargo.getNombreBeneficiario());
                celdaT.setCellStyle(ExcelStyles.createBorderedStyle(book));
                if(cargo.getNombreBeneficiario().length() > max4){
                    max4 = cargo.getNombreBeneficiario().length();
                }
            ...
            JOptionPane.showMessageDialog(null, "Código error: 2140\n" + ex.getMessage(),
                    "Error al obtener inventarios de la BD!!!", JOptionPane.ERROR_MESSAGE);
            logger.ErrorLogger.scribirLog(mensajeError, 2140, UserHome.getUsuario(), ex);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Código error: 2141\n" + ex.getMessage(),
                    "Error al obtener inventarios!!!", JOptionPane.ERROR_MESSAGE);
            logger.ErrorLogger.scribirLog(mensajeError, 2141, UserHome.getUsuario(), ex);
        }
