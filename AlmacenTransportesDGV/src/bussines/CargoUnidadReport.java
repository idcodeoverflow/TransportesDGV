 ...
 * @author David
 */
public class CargoUnidadReport extends ExcelReport{
    
    private Timestamp fechaInicio;
    private Timestamp fechaFin;

    public CargoUnidadReport(javax.swing.JFrame form){
        super(form);
    }
    
    public CargoUnidadReport(){
        super(null);
    }
    ...
    public void generarReporte() {
        ...
        try {
            cargos = accesoCargos.obtenerCargosUnidades(true);
            lazyQ.startLazyQuery();
            for (CargoUnidadDTO cargo : cargos) {
                fila = sheet.createRow(nFila++);

                mensajeError = cargo.toString();

                Cell celdaT = fila.createCell(0);
                celdaT.setCellValue(cargo.getIdCargoUnidad());
            ...
            JOptionPane.showMessageDialog(null, "Código error: 2137\n" + ex.getMessage(),
                    "Error al obtener inventarios de la BD!!!", JOptionPane.ERROR_MESSAGE);
            logger.ErrorLogger.scribirLog(mensajeError, 2137, UserHome.getUsuario(), ex);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Código error: 2138\n" + ex.getMessage(),
                    "Error al obtener inventarios!!!", JOptionPane.ERROR_MESSAGE);
            logger.ErrorLogger.scribirLog(mensajeError, 2138, UserHome.getUsuario(), ex);
        }
